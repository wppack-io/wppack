...
class PostgreSQLDataApiDriverFactory implements DriverFactoryInterface
{
    ...
    public function create(Dsn $dsn, array $options = []): DriverInterface
    {
        ...
        if ($resourceArn === null || $resourceArn === '') {
            throw new ConnectionException('PostgreSQL Data API DSN is missing the resource ARN (host component).');
        }

        $database = ltrim($dsn->getPath() ?? '', '/');
        $secretArn = $dsn->getOption('secret_arn', '');
        if ($secretArn === '') {
            throw new ConnectionException('PostgreSQL Data API DSN is missing the secret_arn option.');
        }
