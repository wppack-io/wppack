...
- **No `search_path` support** — unlike `PgsqlDriver` / `AuroraDsqlDriver`, the Data API is HTTP-stateless, so a `SET search_path` does not persist across calls. If schema scoping is required, issue `SET LOCAL search_path TO ...` inside an explicit `transactionId` scope.
