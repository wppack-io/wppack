...
| `#[AsEventListener]` | `event?: string`, `method?: string`, `priority?: int = 10`, `acceptedArgs?: int = PHP_INT_MAX` | EventDispatcher | PSR-14 イベントリスナーとして登録。クラスまたはメソッドに付与可能（`IS_REPEATABLE`）。デフォルトで全引数を受信 |
