...
| `#[AsDashboardWidget]` | `id: string`, `title: string`, `capability?: string`, `context?: string = 'normal'`, `priority?: string = 'core'` | DashboardWidget | ダッシュボードウィジェットとして登録。`wp_add_dashboard_widget()` にマップ |
...
| `#[AsAdminPage]` | `slug: string`, `title: string`, `menuTitle?: string = ''`, `capability?: string = 'manage_options'`, `parent?: string`, `icon?: string`, `position?: int` | Admin | 管理画面ページとして登録。`add_menu_page()` / `add_submenu_page()` にマップ |
| `#[AsSettingsPage]` | `slug: string`, `title: string`, `menuTitle?: string = ''`, `capability?: string = 'manage_options'`, `optionName?: string = ''`, `optionGroup?: string = ''`, `parent?: string = 'options-general.php'`, `icon?: string`, `position?: int` | Setting | 設定ページとして登録。`add_options_page()` にマップ |
| `#[AsWidget]` | `id: string`, `name: string`, `description?: string = ''` | Widget | ウィジェットとして登録。`register_widget()` にマップ |
| `#[AsFeed]` | `slug: string`, `title?: string = ''` | Feed | カスタムフィードとして登録。`add_feed()` にマップ |
...
| `#[Permission]` | `capability?: string`, `callback?: string`, `public?: bool = false` | `permission_callback` |

`#[RestRoute]` はクラスまたはメソッドに付与可能（`IS_REPEATABLE`）。`#[Param]` はメソッド引数に付与。`#[Permission]` はクラスまたはメソッドに付与可能で、省略時はパブリックアクセスとなる。
