...
モノレポ (wppack/wppack)
  ...
  ├── src/Plugin/SchedulerPlugin/  →  wppack/scheduler-plugin
  └── ...
```
    ...
    strategy:
      matrix:
        package:
          ...
          - { local: 'src/Plugin/SchedulerPlugin', remote: 'wppack/scheduler-plugin' }
    steps:
...
```json
{
    ...
    "autoload": {
        "psr-4": {
            ...
            "WpPack\\Plugin\\SchedulerPlugin\\": "src/Plugin/SchedulerPlugin/src/"
        }
    },
    "replace": {
        "wppack/messenger": "self.version",
        "wppack/scheduler": "self.version",
        "wppack/scheduler-plugin": "self.version"
    }
}
```
