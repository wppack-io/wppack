...
| [Theme Named Hooks](../components/hook/theme.md) | テーマ関連の Named Hook アトリビュート（Hook コンポーネント内） |

> [!NOTE]
> 新規実装では **EventDispatcher** を優先して使用してください。詳細は[プラグイン開発ガイドのイベント・フック登録](plugin-development.md#イベントフック登録)を参照してください。
...
### サービス登録・イベント・フック登録・設定管理

プラグインと同じパターンで `#[AsEventListener]`（推奨）または `#[AsHookSubscriber]`、`#[Env]`、`#[Option]`、`#[Constant]` を使用します。サービスはディレクトリスキャンにより自動登録されるため、個別のアトリビュート指定は不要です。

→ 詳細: [プラグイン開発ガイド](plugin-development.md) のサービス登録・イベント・フック登録・設定管理セクション
...
> [!NOTE]
> 新規実装では EventDispatcher の `#[AsEventListener]` を優先してください。以下の Hook コンポーネントの例は EventDispatcher でも同様に実装できます（`#[AsEventListener(event: 'wp_enqueue_scripts')]` 等）。EventDispatcher パターンの詳細は[プラグイン開発ガイド](plugin-development.md#イベントフック登録)を参照してください。

Hook コンポーネントの Theme Named Hook アトリビュート（`Hook\Attribute\Theme\`）を使って、テーマ固有のフックを宣言的に扱います。
...
→ 詳細: [Theme Named Hooks](../components/hook/theme.md)
...
Hook コンポーネントの [Block Named Hooks](../components/hook/block.md) を使って、ブロックエディタ固有のアセットを管理できます。
