...
WpPack を使って WordPress テーマを一から開発するための実践ガイドです。クラシックテーマと FSE（ブロックテーマ）の両方式について、Kernel・DependencyInjection・Hook・Config を組み合わせた開発方法を解説します。
...
| [Theme](../components/theme.md) | テーマ関連の Named Hook アトリビュート |
...
プラグインと同じパターンで `#[AsService]`、`#[AsHookSubscriber]`、`#[AsConfig]` を使用します。
...
use WpPack\Component\Theme\Attribute\CustomizePreviewInitAction;
...
→ 詳細: [Theme コンポーネント](../components/theme.md)
...
| WpPack 共通 | `ThemeInterface`、`#[AsService]`、`#[AsHookSubscriber]` | 同じ |
...
- **ServiceDiscovery** によるサービスの自動検出
- **#[AsHookSubscriber]** によるフックの宣言的登録
- **#[AsConfig]** による型安全な設定管理
