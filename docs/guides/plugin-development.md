...
WpPack を使って WordPress プラグインを一から開発するための実践ガイドです。Kernel・DependencyInjection・Hook・Config の各コンポーネントを組み合わせて、モダン PHP でプラグインを構築する方法を解説します。
...
| [Config](../components/config/README.md) | 型安全な設定管理 |

必要に応じて以下のコンポーネントも利用できます：

- [Admin](../components/admin.md) — 管理画面メニュー・通知
- [Rest](../components/rest.md) — REST API エンドポイント
- [Command](../components/command.md) — WP-CLI コマンド
- [Plugin](../components/plugin.md) — プラグイン管理画面フック
...
final class MyPlugin implements PluginInterface
{
    ...
    public function register(ContainerBuilder $builder): void
    {
        ...
        // パラメータの設定
        $builder->setParameter('my_plugin.dir', dirname(__DIR__));
        $builder->setParameter('my_plugin.url', plugin_dir_url(dirname(__DIR__) . '/my-plugin.php'));

        // サービスの自動検出
        $discovery = new ServiceDiscovery($builder);
        $discovery->discover(__DIR__, __NAMESPACE__ . '\\');
    }
...
`ServiceDiscovery::discover()` は指定ディレクトリを再帰スキャンし、`#[AsService]` または `#[AsConfig]` アトリビュートを持つクラスを自動登録します。
...
これにより `src/` 配下のすべてのサービスクラスと設定クラスが自動的に ContainerBuilder に登録されます。

### #[AsService] アトリビュート

サービスクラスに `#[AsService]` を付与すると、DI コンテナに登録されます。
...
| パラメータ | デフォルト | 説明 |
|-----------|----------|------|
| `public` | `true` | コンテナから直接取得可能にするか |
| `lazy` | `false` | 遅延初期化を有効にするか |
| `tags` | `[]` | サービスタグ（コンパイラーパスで使用） |
| `autowire` | `true` | コンストラクタ引数の自動注入を有効にするか |

```php
#[AsService(public: false, lazy: true, tags: ['my_plugin.handler'])]
final class HeavyService
{
    // 初回アクセス時にのみインスタンス化される
}
```

### #[Autowire] による依存注入

`#[Autowire]` アトリビュートでコンストラクタ引数に環境変数やパラメータを注入できます。
...
1. クラスに `#[AsService]` と `#[AsHookSubscriber]` を付与
...
1. `ServiceDiscovery::discover()` が `#[AsService]` 付きクラスをコンテナに登録
...
`#[AsConfig]` を付与した readonly クラスで、型安全な設定を定義します。
...
すべてのアトリビュートで、値が存在しない場合はコンストラクタのデフォルト値にフォールバックします。デフォルト値もない場合は `ConfigResolverException` がスローされます。

### 設定クラスの利用

設定クラスは `ServiceDiscovery` によってコンテナに自動登録（`public: true`）されるため、他のサービスからコンストラクタインジェクションで利用できます。
...
→ 詳細: [Config コンポーネント](../components/config/README.md)
...
- **#[AsConfig]** — WordPress options からの型安全な設定読み込み
- **#[AsService] + #[AsHookSubscriber]** — サービスとフックの自動検出・登録
