...
- **環境変数ベースの設定**: `CACHE_DSN` で Redis 接続を構成
...
│   ├── Configuration/
│   │   └── RedisCacheConfiguration.php           ← 設定 VO（CACHE_DSN）
│   └── DependencyInjection/
...
`wp-config.php` で `CACHE_DSN` を定義して Redis を有効にします。
...
final readonly class RedisCacheConfiguration
{
    public function __construct(
        public string $dsn,              // CACHE_DSN（必須）
...
Lambda 等の読み取り専用ファイルシステムでは、プラグイン無効化時にドロップインファイルを削除できません。`CACHE_DSN` が定義済みのままドロップインが残ると、プラグインが無効なのに Redis 接続を試みてしまいます。
