...
EventBridgeSchedulerPlugin は PHP アトリビュートによる宣言的なスケジュール定義を WordPress に導入します。定義されたスケジュールは [Action Scheduler](https://actionscheduler.org/)（Automattic 社製のサードパーティライブラリ）を一次データソースとし、EventBridge Scheduler にリアルタイム同期されます。指定時刻に SQS 経由でメッセージが配信され、Lambda (Bref) 上でハンドラが実行されます。

> [!NOTE]
> Action Scheduler は WordPress コアには含まれていません。WooCommerce に同梱されているほか、単独でも利用可能です。EventBridgeSchedulerPlugin は Action Scheduler をバンドルしています。
...
│                                              │
└──────────────────────────────────────────────┘
            ↓ EventBridgeSchedulerPlugin が収集・同期
┌─ 一次データ ────────────────────────────────┐
...
functions:
  scheduler-worker:
    handler: vendor/wppack/eventbridge-scheduler-plugin/handler.php
    runtime: php-81
    events:
      - sqs:
