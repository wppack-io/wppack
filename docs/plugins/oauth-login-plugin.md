...
- **マルチプロバイダー対応**: Google、Azure AD、GitHub、任意の OIDC プロバイダーを同時に利用可能
...
| パラメータ | 説明 |
|-----------|------|
| `type` | プロバイダー種別（`google`, `azure`, `github`, `oidc`） |
...
| `tenant_id` | - | Azure AD テナント ID（`azure` タイプで必須） |
...
| `OAUTH_BASE_PATH` | `/oauth` | OAuth エンドポイントのベースパス |
...
2. リダイレクト URI: `https://your-site.com/oauth/azure/callback`（Web プラットフォーム）
...
ベースパスはデフォルトで `/oauth` です。`OAUTH_BASE_PATH` で変更可能です。
