...
// DB index を指定
define('CACHE_DSN', 'redis://127.0.0.1:6379/2');

// パスワード認証
define('CACHE_DSN', 'redis://secret@127.0.0.1:6379');
...
// ソケット + パスワード + DB index
define('CACHE_DSN', 'redis://secret@/var/run/redis.sock/2');

// Cluster
define('CACHE_DSN', 'redis:?host[node1:6379]&host[node2:6379]&host[node3:6379]&redis_cluster=1');
...
```php
define('CACHE_DSN', 'redis:?host[node1:6379]&host[node2:6379]&redis_cluster=1&failover=distribute');
```
...
```php
define('CACHE_DSN', 'redis://master-pass@?host[sentinel1:26379]&host[sentinel2:26379]&redis_sentinel=mymaster');
```
...
// DSN クエリパラメータで指定
define('CACHE_DSN', 'redis://127.0.0.1:6379?class=Predis%5CClient');
```
...
define('CACHE_DSN', 'valkeys://my-cache-xxxxx.serverless.apne1.cache.amazonaws.com:6379?redis_cluster=1');

// 認証トークンを使用する場合
define('CACHE_DSN', 'valkeys://my-auth-token@my-cache-xxxxx.serverless.apne1.cache.amazonaws.com:6379?redis_cluster=1');
```
...
// プライマリエンドポイント（TLS なし）
define('CACHE_DSN', 'redis://my-cluster.xxxxx.0001.apne1.cache.amazonaws.com:6379');

// TLS 有効
define('CACHE_DSN', 'rediss://my-cluster.xxxxx.0001.apne1.cache.amazonaws.com:6379');

// 認証トークン + TLS
define('CACHE_DSN', 'rediss://my-auth-token@my-cluster.xxxxx.0001.apne1.cache.amazonaws.com:6379');
```
...
define('CACHE_DSN', 'redis:?host[my-cluster.xxxxx.clustercfg.apne1.cache.amazonaws.com:6379]&redis_cluster=1');

// TLS + 認証
define('CACHE_DSN', 'rediss:?host[my-cluster.xxxxx.clustercfg.apne1.cache.amazonaws.com:6379]&redis_cluster=1&auth=my-auth-token');

// フェイルオーバー戦略（リードレプリカにリード分散）
define('CACHE_DSN', 'redis:?host[my-cluster.xxxxx.clustercfg.apne1.cache.amazonaws.com:6379]&redis_cluster=1&failover=distribute');
```
