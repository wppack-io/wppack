...
PSR-3 準拠のため、`LoggerInterface` のタイプヒントを変更せずに Monolog 等の実装に差し替えられます。ファクトリクラスを作成して DI コンテナに登録します：
...
// ファクトリを登録し、LoggerInterface として Monolog を提供
$builder->register(MonologFactory::class);
$builder->register(LoggerInterface::class)
    ->setFactory([new Reference(MonologFactory::class), 'create']);
```
