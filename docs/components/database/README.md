...
| PostgreSQL | PostgreSQLDriver | `pg_query_params()` / `pg_prepare()` + `pg_execute()` | `?` → `$1, $2, ...` 自動変換 |
| Aurora MySQL Data API | MySQLDataApiDriver | HTTP API `parameters` フィールド | `?` → `:param1, :param2, ...` 自動変換 |
| Aurora PostgreSQL Data API | PostgreSQLDataApiDriver | HTTP API `parameters` フィールド | `?` → `:param1, :param2, ...` 自動変換 |
| Aurora DSQL | AuroraDSQLDriver | PostgreSQLDriver 継承 | `?` → `$1, $2, ...` 自動変換 |
...
- **SQL_CALC_FOUND_ROWS + パラメタ化 LIMIT**: `WP_Query` / `WP_User_Query` は `LIMIT %d, %d` で LIMIT 値を prepared statement パラメタとして渡します。`FOUND_ROWS()` 用の COUNT(\*) サブクエリは元 SQL から末尾 LIMIT を除去するため、除去された placeholder 分だけパラメタ配列も切り詰めてから driver に渡します。この切り詰めを忘れるとパラメタ数の不整合で COUNT(\*) が失敗し、ページネーションが 0 件になる回帰があったので、末尾 LIMIT（コメント／セミコロン混在を含む）の除去と placeholder 数の再計算を連動させています。
...
| PostgreSQL | `'pgsql'` | PostgreSQLPlatform | PostgreSQLDriver, PostgreSQLDataApiDriver |
...
DriverInterface（SPI）
    ...
    ├── PostgreSQLDriver（Bridge/PostgreSQL）
    ├── MySQLDataApiDriver（Bridge/MySQLDataApi）← MySQLDriver 継承
    ├── PostgreSQLDataApiDriver（Bridge/PostgreSQLDataApi）← PostgreSQLDriver 継承
    └── AuroraDSQLDriver（Bridge/AuroraDSQL）← PostgreSQLDriver 継承
```
...
Connection を省略した場合、`DatabaseManager` は `$wpdb` から自動検出して Connection を構築します。`WPPackWpdb`（db.php drop-in 経由）であれば writer Driver を再利用し、**そのエンジンのトランスレーターも Connection に注入します**。これにより `$db->executeStatement('CREATE TABLE ... AUTO_INCREMENT')` のような MySQL 方言 DDL も、SQLite / PostgreSQL / Aurora DSQL バックエンドで正しく変換されて実行されます。`$wpdb` 経由のクエリだけでなく、`DatabaseManager` の生クエリもすべて同じ方言変換パスを通ります。
...
`setConnection()` なしの場合は自動検出に切り替わります。`WPPackWpdb`（db.php drop-in）が使われていれば writer driver を再利用し、それ以外の素の `wpdb` なら既存 `mysqli` をラップするか `DB_*` 定数から MySQLDriver を構築します。いずれの場合も driver のクエリトランスレーターを Connection に注入するため、MySQL 方言のクエリ／DDL が `executeQuery()` / `executeStatement()` 経由でも正しく非 MySQL エンジンに変換されます。
...
| `pgsql://` | PostgreSQLDriver | `pgsql://user:pass@host:5432/dbname` |
| `mysql+dataapi://` | MySQLDataApiDriver | `mysql+dataapi://cluster-arn/dbname?secret_arn=...` |
| `pgsql+dataapi://` | PostgreSQLDataApiDriver | `pgsql+dataapi://cluster-arn/dbname?secret_arn=...` |
...
| PostgreSQLDriver | PostgreSQLQueryTranslator | MySQL → PostgreSQL 変換（AST ベース） |
| MySQLDataApiDriver | NullQueryTranslator | パススルー（Aurora MySQL 互換） |
| PostgreSQLDataApiDriver | PostgreSQLQueryTranslator | MySQL → PostgreSQL 変換（AST ベース） |
...
- `SHOW TABLES` / `SHOW COLUMNS` 等のメタ情報クエリは `table_schema = 'public'` ではなく `current_schema()` を参照するため、`search_path` の先頭スキーマの中を走査する（マルチテナント環境で tenant スキーマに切り替えた場合もそのスキーマが見える）

**PostgreSQL — スキーマ / search_path:**

`PostgreSQLDriver` は接続直後に `SET search_path TO ...` を発行するオプションを提供する。マルチテナントや blog 毎にスキーマを分離したい場合に使用する。

| 指定方法 | 例 |
|---------|-----|
| DSN `search_path`（順序付きリスト） | `pgsql://.../mydb?search_path=tenant_42,public` |
| DSN `schema`（単一スキーマのエイリアス） | `pgsql://.../mydb?schema=tenant_42` |
| コンストラクタ引数 | `new PostgreSQLDriver(..., searchPath: ['tenant_42', 'public'])` |

- 各スキーマ名は `quote_ident()` 相当のダブルクォートで保護される（大文字・予約語・非 ASCII 文字も扱える）
- NUL / 改行 / CR を含むスキーマ名は `ConnectionException` を即座に throw（libpq の C-string 切り詰めを防ぐ）
- `searchPath = null`（未指定）時はサーバー側のデフォルト `"$user", public` がそのまま使われる
- `gone-away` 検出後の再接続でも自動的に再適用されるため、`wait_timeout` 経由の透過再接続で search_path が失われることはない
- Aurora DSQL も `PostgreSQLDriver` を継承しているため同じ DSN オプションで有効。DSQL の `doConnect()` は IAM トークン再生成のために独自の接続文字列組み立てを行うが、`applySearchPath()` を明示的に呼び出して整合性を保つ
- **Aurora PostgreSQL Data API は未対応**：HTTP ステートレスで接続スコープを持たないため `SET search_path` が次リクエストに残らない。必要なら `BEGIN TRANSACTION` で得られる `transactionId` 内で明示的に `SET LOCAL search_path TO ...` を発行するのが回避策
...
- **トークン有効期間:** デフォルト 900 秒（15分）。期限 120 秒前に自動再接続（WP-CLI 等の長時間プロセス対応。SigV4 クロックスキュー + async-aws の credential 取得 latency + `pg_connect()` ラウンドトリップをカバー）
- **SSL verify-full:** 必須。`PostgreSQLDriver` の `sslmode` パラメータで強制
- **OCC リトライ:** 楽観的同時実行制御。SQLSTATE `40001` / `OC000` / `OC001` で指数バックオフ + decorrelated jitter（初期 100ms, 2 倍, 上限 5s、jitter は `random_int(waitMs/2, waitMs)`）。`occMaxRetries` で設定（デフォルト 3）
- **search_path / schema:** `PostgreSQLDriver` から継承し、DSN の `?search_path=tenant_42,public` または `?schema=myschema` を指定するとトークン再生成後の再接続でも自動的に再適用される
- **DSN パラメータ:** `dsql://admin@cluster.dsql.us-east-1.on.aws/postgres?occMaxRetries=5&tokenDurationSecs=600&region=us-east-1&search_path=tenant_42,public`
...
| `wppack/postgresql-database` | PostgreSQL ドライバ | ext-pgsql |
| `wppack/mysql-data-api-database` | Aurora MySQL Data API ドライバ | async-aws/rds-data-service |
| `wppack/postgresql-data-api-database` | Aurora PostgreSQL Data API ドライバ | wppack/postgresql-database, async-aws/rds-data-service |
| `wppack/aurora-dsql-database` | Aurora DSQL ドライバ | wppack/postgresql-database, async-aws/core |
...
| PostgreSQL | `pgsql://user:pass@host:5432/mydb` | wppack/postgresql-database |
...
| Aurora PostgreSQL Data API | `pgsql+dataapi://cluster-arn/mydb?secretArn=...&region=...` | wppack/postgresql-data-api-database |
...
`PostgreSQLDriver` は同等の仕組みで `server closed the connection`, `terminating connection due to administrator command`, `SSL SYSCALL error` といった fatal disconnect 系エラー、および `pg_connection_status !== CONNECTION_OK` を検出して handle を drop します。Aurora DSQL/RDS のフェイルオーバーや `pg_terminate_backend()` 後も透過的に再接続します。
...
new PostgreSQLDriver(host: 'localhost', ..., persistent: true);  // pg_pconnect を使用
```
...
- **`lastInsertId()`:** HTTP-stateless のため session 間で `SELECT lastval()` は undefined になります。PostgreSQLDataApiDriver は失敗を suppress して 0 を返します。
