...
`PgsqlDriver` は接続直後に `SET search_path TO ...` を発行するオプションを提供する。マルチテナントや blog 毎にスキーマを分離したい場合に使用する。

| 指定方法 | 例 |
|---------|-----|
| DSN `search_path`（順序付きリスト） | `pgsql://.../mydb?search_path=tenant_42,public` |
| DSN `schema`（単一スキーマのエイリアス） | `pgsql://.../mydb?schema=tenant_42` |
| コンストラクタ引数 | `new PgsqlDriver(..., searchPath: ['tenant_42', 'public'])` |
...
- Aurora DSQL も `PgsqlDriver` を継承しているため同じ DSN オプションで有効。DSQL の `doConnect()` は IAM トークン再生成のために独自の接続文字列組み立てを行うが、`applySearchPath()` を明示的に呼び出して整合性を保つ
...
- **search_path / schema:** `PgsqlDriver` から継承し、DSN の `?search_path=tenant_42,public` または `?schema=myschema` を指定するとトークン再生成後の再接続でも自動的に再適用される
