...
- **関数:** NOW, CURDATE, UNIX\_TIMESTAMP, DATE\_ADD/SUB, DATE\_FORMAT, MONTH/YEAR/DAY, CONCAT, LEFT/RIGHT, IF, IFNULL, RAND, GREATEST/LEAST, LOCATE 等 40+関数
- **文:** INSERT IGNORE, REPLACE INTO, ON DUPLICATE KEY UPDATE, LIMIT, FOR UPDATE, TRUNCATE
- **DDL:** CREATE TABLE 型変換, AUTO\_INCREMENT, ENGINE/CHARSET 除去, ALTER TABLE ADD COLUMN
- **SHOW:** TABLES, COLUMNS, CREATE TABLE, INDEX, VARIABLES, DATABASES, DESCRIBE

全変換リストと設計判断の詳細は [Query Translation Architecture](./query-translation.md) を参照。
