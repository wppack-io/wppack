...
|-------|--------|------------|
| `NOW()` | `datetime('now')` | そのまま |
| `CURDATE()` | `date('now')` | `CURRENT_DATE` |
| `CURTIME()` | `time('now')` | `CURRENT_TIME` |
| `UNIX_TIMESTAMP()` | `strftime('%s','now')` | `EXTRACT(EPOCH FROM NOW())::INTEGER` |
| `UTC_TIMESTAMP()` | `datetime('now')` | `NOW() AT TIME ZONE 'UTC'` |
| `FROM_UNIXTIME(t)` | `datetime(t, 'unixepoch')` | `TO_TIMESTAMP(t)` |
| `DATE_ADD(d, INTERVAL n unit)` | `datetime(d, '+n unit')` | `d + INTERVAL 'n unit'` |
| `DATE_SUB(d, INTERVAL n unit)` | `datetime(d, '-n unit')` | `d - INTERVAL 'n unit'` |
| `DATE_FORMAT(d, fmt)` | `strftime(fmt, d)` | `TO_CHAR(d, fmt)` |
| `DATEDIFF(d1, d2)` | `julianday(d1) - julianday(d2)` | `DATE_PART('day', d1 - d2)` |
| `MONTH(d)` / `YEAR(d)` / `DAY(d)` | `strftime('%m', d)` 等 | `EXTRACT(MONTH FROM d)` 等 |
| `HOUR(d)` / `MINUTE(d)` / `SECOND(d)` | `strftime('%H', d)` 等 | `EXTRACT(HOUR FROM d)` 等 |
| `DAYOFWEEK(d)` | `strftime('%w', d) + 1` | `EXTRACT(DOW FROM d) + 1` |
| `RAND()` | `random()` | `random()` |
| `IFNULL(a, b)` | そのまま | `COALESCE(a, b)` |
| `IF(cond, t, f)` | `CASE WHEN cond THEN t ELSE f END` | 同左 |
| `CONCAT(a, b, ...)` | `a \|\| b \|\| ...` | そのまま |
| `CONCAT_WS(sep, a, b)` | `a \|\| sep \|\| b` | そのまま |
| `LEFT(s, n)` | `SUBSTR(s, 1, n)` | `SUBSTRING(s FROM 1 FOR n)` |
| `RIGHT(s, n)` | `SUBSTR(s, -n)` | そのまま |
| `SUBSTRING(s, p, n)` | `SUBSTR(s, p, n)` | そのまま |
| `CHAR_LENGTH(s)` | `LENGTH(s)` | `LENGTH(s)` |
| `LOCATE(sub, str)` | `INSTR(str, sub)` | `POSITION(sub IN str)` |
| `LCASE(s)` / `UCASE(s)` | `lower(s)` / `upper(s)` | 同左 |
| `GREATEST(a, b)` | `MAX(a, b)` | そのまま |
| `LEAST(a, b)` | `MIN(a, b)` | そのまま |
| `CAST(x AS SIGNED)` | `CAST(x AS INTEGER)` | 同左 |
| `LAST_INSERT_ID()` | `last_insert_rowid()` | `lastval()` |
| `GROUP_CONCAT(col SEP s)` | — | `STRING_AGG(col, s)` |
| `REGEXP` | そのまま（UDF） | `~*` 演算子 |
| `VERSION()` | `'10.0.0-wppack'` | `version()` |
| `DATABASE()` | `'main'` | `CURRENT_DATABASE()` |
...
| MySQL | SQLite | PostgreSQL |
|-------|--------|------------|
| `SHOW TABLES` | `sqlite_master` クエリ | `information_schema.tables` |
| `SHOW COLUMNS FROM t` | `PRAGMA table_info(t)` | `information_schema.columns` |
| `SHOW CREATE TABLE t` | `sqlite_master` SQL 列 | — |
...
**SQLite:**
- `ALTER TABLE DROP COLUMN` / `MODIFY COLUMN` は SQLite の制限によりスキップ（ADD COLUMN と RENAME のみ対応）
...
```php
// wp-config.php — 明示的に DSN を指定
define('DATABASE_DSN', 'mysql://user:pass@host:3306/mydb');
```
