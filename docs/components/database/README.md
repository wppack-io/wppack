...
- `REGEXP` は PHP ユーザー定義関数（`preg_match` ベース）として自動登録
- `CONCAT()`, `CONCAT_WS()`, `CHAR_LENGTH()`, `FIELD()` も UDF として登録
- `ON DUPLICATE KEY UPDATE` → `ON CONFLICT DO UPDATE SET`（`VALUES(col)` → `excluded.col`）
...
- `REGEXP` → `~*` 演算子に変換
- `IFNULL()` → `COALESCE()`
- `DATE_FORMAT()` → `TO_CHAR()` + フォーマット文字列変換（`%Y→YYYY`, `%m→MM` 等）
- `DATETIME` → `TIMESTAMP`, `BLOB` → `BYTEA`, `JSON` → `JSONB`
- `TINYINT(1)` → `BOOLEAN`
