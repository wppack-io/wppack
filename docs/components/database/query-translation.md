...
| UNHEX | UDF | **UDF** | WpPack: `hex2bin()` UDF |
| FROM\_BASE64 / TO\_BASE64 | UDF | **UDF** | WpPack: `base64_decode/encode()` UDF |
| INET\_ATON / INET\_NTOA | UDF | **UDF** | WpPack: `ip2long/long2ip()` UDF |
| GET\_LOCK / RELEASE\_LOCK | UDF (no-op) | **UDF** (no-op) | 同方式 |
...
| ALTER TABLE CHANGE COLUMN | ✅ (テーブル再作成) | ✅ (テーブル再作成) |
| ALTER TABLE RENAME | ✅ | ✅ |
...
| SHOW FULL TABLES | ✅ | ✅ |
| SHOW COLUMNS FROM | ✅ | ✅ |
| SHOW CREATE TABLE (MySQL 互換 DDL 再構築) | ✅ | ✅ (pragma + キャッシュ) |
