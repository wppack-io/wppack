...
Aurora DSQL は PostgreSQL ワイヤプロトコル互換だが、SQL サポートは
PostgreSQL のサブセット。`AuroraDSQLQueryTranslator` は
`PostgreSQLQueryTranslator` を内包し、**MySQL → DSQL** の変換を主軸に
設計されている。

方針:

- MySQL / wpdb の一般的な idiom は silent に等価な DSQL 形に落とす
  （外部キーや `ON UPDATE CURRENT_TIMESTAMP` のように MySQL 側で日常的に
  使われる構文は、DSQL にそのまま渡せなくても翻訳を継続する）。
- 「DSQL で実行不能だが silent に落とせない」構文（ユーザが明示的に
  書いた一時テーブル・トリガ・関数・破壊的 ALTER TABLE）は
  `UnsupportedFeatureException` を投げる。

### silent に書き換え／ドロップ

| MySQL / PostgreSQL 入力 | Aurora DSQL 出力 | 説明 |
|-------|-----------|------|
| `TRUNCATE TABLE t` | `DELETE FROM "t"` + sequence reset | DSQL は TRUNCATE 未対応 |
| `CREATE [UNIQUE] INDEX …` | `CREATE [UNIQUE] INDEX ASYNC …` | DSQL は非同期インデックス構築のみサポート |
| `CREATE TABLE t (id INT AUTO_INCREMENT …)` | `… INTEGER GENERATED BY DEFAULT AS IDENTITY …` | `SERIAL`/`BIGSERIAL`/`SMALLSERIAL` は未対応。`GENERATED AS IDENTITY` に置換 |
| `CREATE TABLE … FOREIGN KEY …` / `col … REFERENCES …` | FK 制約は落とす | DSQL は外部キー未対応。MyISAM 互換で silent ドロップ（参照先の整合性はアプリ側責任） |
| `CREATE TABLE … ON UPDATE CURRENT_TIMESTAMP …` | DEFAULT だけ残し、`PostgreSQLQueryTranslator` が生成する `CREATE FUNCTION`/`CREATE TRIGGER` artifact は落とす | DSQL にトリガ・PL/pgSQL なし。`updated_at` の UPDATE 更新はアプリケーション側責任 |

`CREATE INDEX ASYNC` は即座に制御を返し、バックグラウンドで構築される。
完了を待つ場合は `sys.wait_for_job(<job_id>)` を使う
（[AWS ドキュメント][dsql-create-index]）。

### UnsupportedFeatureException を投げるケース

silent に落とすと「ユーザが書いた明示的な意図」と乖離するものは拒否する。

| 入力 | 理由 |
|------|------|
| `CREATE TEMPORARY TABLE …` | 一時テーブル未対応。silent 変換すると永続テーブルとして作られてしまう |
| `CREATE TRIGGER …` | トリガ未対応。ユーザが明示的にトリガを書いた場合は拒否 |
| `CREATE FUNCTION …` / `CREATE PROCEDURE …` | ユーザ定義関数・プロシージャ未対応（PL/pgSQL 含む） |
| `ALTER TABLE … DROP COLUMN …` | カラム削除未対応 |
| `ALTER TABLE … ALTER COLUMN c (SET DATA TYPE \| TYPE \| SET DEFAULT \| DROP DEFAULT \| SET NOT NULL \| DROP NOT NULL)` | 既存カラムの型・デフォルト・NOT NULL 変更は未対応 |
| `ALTER TABLE … MODIFY COLUMN …` / `CHANGE COLUMN …` | 同上（MySQL 構文） |

#### カラム変更・削除の移行パターン

DSQL は既存カラムの in-place 変更をサポートしない（型変更・デフォルト
変更・nullability 変更・DROP COLUMN いずれも不可）。必要な移行は次の 4 段
階をアプリ側で実行する:

1. 既存テーブルを `ALTER TABLE t RENAME TO t_bak` でリネーム
2. 新スキーマで `CREATE TABLE t` を実行
3. `INSERT INTO t (col_a, col_b, …) SELECT col_a, col_b, … FROM t_bak` で
   データをフルコピー
4. 確認後に `DROP TABLE t_bak`

DSQL は 1 トランザクションで DDL と DML を混在できないため、各ステップは
独立したトランザクションで実行する必要がある。
...
### Aurora DSQL 公式ドキュメント

- [PostgreSQL 互換性の概要][dsql-compat]
- [サポートされる SQL 機能][dsql-sql-support]
- [未対応機能一覧][dsql-unsupported]
- [`CREATE INDEX ASYNC` 構文][dsql-create-index]
- [OCC（楽観的同時実行制御）モデル][dsql-concurrency]

[dsql-compat]: https://docs.aws.amazon.com/aurora-dsql/latest/userguide/working-with-postgresql-compatibility.html
[dsql-sql-support]: https://docs.aws.amazon.com/aurora-dsql/latest/userguide/working-with-postgresql-compatibility-sql-support.html
[dsql-unsupported]: https://docs.aws.amazon.com/aurora-dsql/latest/userguide/working-with-postgresql-compatibility-unsupported-features.html
[dsql-create-index]: https://docs.aws.amazon.com/aurora-dsql/latest/userguide/working-with-postgresql-compatibility-sql-support.html
[dsql-concurrency]: https://docs.aws.amazon.com/aurora-dsql/latest/userguide/concurrency-control.html
