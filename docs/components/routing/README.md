...
```php
public function __invoke(): ?RouteResponse
{
...
class EventController
{
    ...
    public function archive(): TemplateResponse
    {
        return new TemplateResponse(
            get_template_directory() . '/templates/event-archive.php',
            [
                'event_year' => (int) get_query_var('event_year'),
                'event_month' => (int) get_query_var('event_month'),
            ],
        );
    }
...
- **HttpFoundation Component** — `Request` 注入とパラメータ自動解決に使用
