...
WordPress のメール送信をモダン化するコンポーネント。PHPMailer を継承した `WpPackPhpMailer` で WordPress のグローバル `$phpmailer` を差し替え、DSN ベースのトランスポート設定とオブジェクト指向の Email API を提供します。
...
WordPress が wp_mail_from, wp_mail_content_type 等のフィルターを適用
  ↓
Mailer::onPhpMailerInit()  ← phpmailer_init でトランスポート設定
  ↓
WpPackPhpMailer::send() → preSend() → postSend()
  ↓
wp_mail_succeeded / wp_mail_failed
```

サードパーティプラグインや既存のテーマが使う `wp_mail()` がそのまま設定したトランスポート（SES 等）経由になります。
...
$mailer->send($email)
  ↓
WpPackPhpMailer のメッセージ状態をクリア（clearAllRecipients 等）
  ↓
Email の内容で PHPMailer を設定（populatePhpMailer）
  ↓
transport->configure() でトランスポート設定
  ↓
...
`wp_mail()` を呼ばずに PHPMailer を直接操作。`phpmailer_init` 以降の WordPress フック（`wp_mail_succeeded`, `wp_mail_failed`）は発火するため、ログプラグイン等との互換性を維持します。
...
| `smtp://user:pass@host:port?encryption=tls` | `smtp` | 汎用 SMTP |
| `null://default` | `null` | テスト用 no-op |

### サードパーティトランスポート

`wppack/amazon-mailer` パッケージで Amazon SES トランスポートが提供されます。詳細は [amazon-mailer.md](amazon-mailer.md) を参照。
...
PHPMailer を拡張し、カスタムメーラーを登録できる汎用クラス。
...
| トランスポート | カスタムメーラー | 動作 |
|--------------|----------------|------|
| NativeTransport | 登録なし、設定なし | `parent::postSend()` → `mailSend()` |
| SmtpTransport | 登録なし、`isSMTP()` 設定 | `parent::postSend()` → `smtpSend()` |
| NullTransport | `null` → no-op | 送信なし |
| SesTransport | `ses` → SES Raw API | Content.Raw で送信 |
| SesApiTransport | `sesapi` → SES Simple API | Content.Simple で送信 |
| SesSmtpTransport | 登録なし、SES SMTP 設定 | `parent::postSend()` → `smtpSend()` |
...
- `configure()`: PHPMailer にトランスポート固有の設定を適用（カスタムメーラー登録含む）
- PHPMailer インスタンスの生成は `Mailer` が管理（Transport の責務外）

### AbstractTransport

カスタムメーラーを使うトランスポートの基底クラス。`configure()` でカスタムメーラーの登録と `$Mailer` プロパティの設定を行います。
...
abstract class AbstractTransport implements TransportInterface
{
    ...
    public function configure(WpPackPhpMailer $phpMailer): void
    {
        $phpMailer->registerCustomMailer($this->getMailerName(), fn(WpPackPhpMailer $m): bool => ...);
...
`$mailer->send()` の戻り値。メッセージ ID と Envelope を取得できます。
...
| `phpmailer_init` | action | PHPMailer 設定を変更 |
