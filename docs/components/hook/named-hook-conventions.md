...
加えて、汎用基底クラス `Action` / `Filter` と、AJAX フック（`WpAjaxAction` / `WpAjaxNoprivAction`）を提供します。
...
use Attribute;
use WpPack\Component\Hook\Attribute\Action as BaseAction;

#[Attribute(Attribute::TARGET_METHOD | Attribute::IS_REPEATABLE)]
final class SavePostAction extends BaseAction
{
    public function __construct(
        public ?string $postType = null,
        int $priority = 10,
    ) {
        $hook = $this->postType !== null
            ? "save_post_{$this->postType}"
            : 'save_post';

        parent::__construct(hook: $hook, priority: $priority);
    }
}
```
...
use Attribute;
use WpPack\Component\Hook\Attribute\Filter as BaseFilter;

#[Attribute(Attribute::TARGET_METHOD | Attribute::IS_REPEATABLE)]
final class PostsWhereFilter extends BaseFilter
{
    public function __construct(
        int $priority = 10,
    ) {
        parent::__construct(hook: 'posts_where', priority: $priority);
    }
}
```
...
|--------------|------------------------|
| **Hook** | `InitAction`, `AdminInitAction`, `PluginsLoadedAction`, `AfterSetupThemeAction`, `WpLoadedAction`, `WpAjaxAction`, `WpAjaxNoprivAction` |
| **PostType** | `SavePostAction`, `DeletePostAction`, `TransitionPostStatusAction` |
...
| **Ajax** | `WpAjaxAction`, `WpAjaxNoprivAction`（Hook component 提供） |
...
| **Security** | `WpLoginAction`, `AuthenticateFilter` 等 |
| **Nonce** | `NonceLifeFilter` 等 |
| **Routing** | `RewriteRulesArrayFilter`, `TemplateRedirectAction` 等 |
...
| **Plugin** | `PluginsLoadedAction`, `PluginActionLinksFilter` 等 |
| **NavigationMenu** | `WpNavMenuArgsFilter`, `NavMenuCssClassFilter` 等 |
| **Comment** | `CommentPostAction`, `TransitionCommentStatusAction` 等 |
...
| **DashboardWidget** | `WpDashboardSetupAction` 等 |
