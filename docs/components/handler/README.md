...
`run()` メソッドが以下のライフサイクルを一貫して管理する:

1. Request がなければ `createFromGlobals()` で生成
2. 環境セットアップ（Lambda ディレクトリ作成等）
3. プロセッサチェーンによるリクエスト処理
4. 静的ファイルやリダイレクトは即座にレスポンス送信し `null` を返す
5. PHP ファイルの場合は `$_SERVER` 変数を準備
6. Kernel が利用可能なら `Kernel::create($request)` で Request を引き渡し
7. 解決されたファイルパスを返す — 呼び出し元がグローバルスコープで `require` する
