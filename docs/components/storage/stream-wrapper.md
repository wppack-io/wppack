...
| 関数 | 説明 | 備考 |
|------|------|------|
| `file_exists()` | ファイル/ディレクトリの存在確認 | `url_stat` → `exists()` |
...
| `is_dir()` | ディレクトリかどうか判定 | `url_stat` の結果から判定 |
...
| 関数 | 説明 | 備考 |
|------|------|------|
| `mkdir()` | ディレクトリを作成 | no-op（後述） |
| `rmdir()` | ディレクトリを削除 | no-op（後述） |
...
拡張子を持たないパスは高い確率でディレクトリ（プレフィックス）として使われます。StatCache はこのパターンを検出し、`listContents()` で存在確認を行うことで、不要な HEAD リクエストを回避します。

```php
// 拡張子なし → ディレクトリとして最適化された判定
is_dir('s3://uploads/2024/01');      // listContents() で確認（HEAD リクエストなし）

// 拡張子あり → 通常のオブジェクト存在確認
is_file('s3://uploads/2024/01/photo.jpg');  // HEAD リクエスト
```

## mkdir / rmdir の no-op 動作

オブジェクトストレージではディレクトリは仮想的な概念です。S3 や GCS にはディレクトリという実体は存在せず、キーのプレフィックスでディレクトリ構造が表現されます。

そのため、`mkdir()` と `rmdir()` は成功を返しますが、実際にはストレージ操作を行いません（no-op）。

```php
// 成功（true）を返すが、ストレージには何も作成されない
mkdir('s3://uploads/2024/01', 0777, true);

// 成功（true）を返すが、ストレージからは何も削除されない
rmdir('s3://uploads/2024/01');
```

WordPress コアはアップロード時に `mkdir()` を呼び出すため、no-op で互換性を保つ必要があります。
