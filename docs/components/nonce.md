...
| `checkAjaxReferer(string $action, string $name = '_wpnonce'): void` | `check_ajax_referer()` | AJAX リファラーチェック |
...
class AjaxHandler
{
    ...
    {
        wp_enqueue_script('my-ajax', plugin_dir_url(__FILE__) . 'js/ajax.js', ['jquery']);

        wp_localize_script('my-ajax', 'myAjax', [
            'ajaxurl' => admin_url('admin-ajax.php'),
            'nonce' => $this->nonces->create('my-ajax-action'),
        ]);
    }
    ...
    {
        $this->nonces->checkAjaxReferer('my-ajax-action', 'nonce');

        // 検証成功（失敗時は checkAjaxReferer が wp_die() を呼ぶ）
...
$nonces->checkAjaxReferer('action', 'nonce');  // AJAX リファラーチェック
```
