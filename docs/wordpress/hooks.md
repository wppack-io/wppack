...
> **注意**: `$callback` 指定時は優先度 `0` が返る場合があります。`has_filter() === false` で比較してください。
...
> **注意**: `spl_object_hash()` ベースの ID は、オブジェクトが破棄されると同じ ID が別のオブジェクトに再利用される可能性があります。`remove_filter()` はオブジェクトのライフタイム内に呼ぶ必要があります。
